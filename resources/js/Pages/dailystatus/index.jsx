...
const Timesheet = ({ leave, user, user_type, permissions ,notif,userid}) => {
...
useEffect(() => {
  ...
  const handleDescriptionChange = (index, date, value) => {
    ...
    axios.post('timesheets-store', { timesheets: [updatedEntry] })
        .then(response => {
        ...
        .catch(error => {
            ...
            if (error.response && error.response.data.errors) {
                Object.values(error.response.data.errors).forEach(errorMsg => notyf.error(errorMsg));
            } else {
...
const handleTimeChange = async (index, date, value) => {
    ...
    newWeekData[index].disabled = isTaskComplete || status === 1 || isDisabled(date) ||
        (isHoliday(date) && !isAssignedToWorkOnHoliday(date, empl.id)) || isWeekend(date);
...
const handleStatus = (e) => {
    ...
    post('timesheets-status', {
        ...
        onSuccess: () => {
            notyf.success('Timesheets status updated successfully please contact admin ');
            window.location.reload();
        },
        onError: () => {
            console.error('Error updating the timesheets status');
            notyf.error('An error occurred while updating the timesheets status.');
        },
    });
};
...
const [nonBillableTimesheets, setNonBillableTimesheets] = useState(Array(7).fill({ title: '', time: '' }));
...
<div className='px-4'>
    ...
    <form onSubmit={(e) => e.preventDefault()} >
        ...
        max={100} // Ensure maximum value is 100
                                            ...
                                            <td>{
                                                status === 1 ?'': <button onClick={() => handleRemove(index)}><MdDelete className="text-red-500"/></button>
                                            }
...
<p>Weekly Overtime: {totals.projectWise[projectId].weeklyOvertime}</p>
    ...
    <button type="button" onClick={handleStatus}>Update Status</button>
        ...
        <div className='flex justify-between   bg-[#356A6A]'>
            ...
            {
    isSubmitDisabledss ? (
      <button  className=' p-2 bg-[#777475] text-white rounded-md' disabled={status === 1}>Submit time Status</button>
    ) : (
  ...
  <div className="mt-3">
                        ...
                        ):is_approved === 0 ?<span className="font-bold text-red-600">Timesheet Rejected</span>:''}
                    </div>
                {
