...
const DashboardDeals = ({ user, user_type, notif }) => {
  const dealsData = [
  ...
  const filteredDeals = currentDeals.filter(
    (deal) =>
  ...
  const handleDelete = (e, id) => {
    setCurrentDeals(currentDeals.filter((deal) => deal.id !== id));
  };
          ...
          <table className="table w-full p-4 border">
            <thead className="border bg-[#0A1B3F] text-white">
              <tr>
                ...
                <th className="p-3 text-left border">Users</th>
              ...
              {currentDealsToShow.length > 0 ? (
                currentDealsToShow.map((deal, index) => (
                  <tr key={deal.id}>
                    ...
                    <td className="p-3 border">{deal.tasks}</td>
