...
const Department = ({ user, notif, user_type, payslips }) => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [selectedBranchId, setSelectedBranchId] = useState(null);
    // const [payslips, setBracnches] = useState(null);
    ...
    const fetchBranches = async () => {
        try {
            const response = await axios.get('/payslips'); // Adjust the API endpoint
    ...
    if (confirm('Are you sure you want to delete this branch?')) {
        try {
            await axios.delete(`/payslips/${id}`); // Make sure to await the delete call
            ...
            <div className="flex px-9">
                ...
                <div className="flex-1 p-6 bg-gray-100">
                    <div className='flex justify-between'>
                        <h1 className="mb-4 text-2xl font-bold">Manage payslips</h1>
    ...
    <table className='min-w-full border border-gray-300'>
        <thead className='bg-gray-200'>
            <tr>
                <th className='px-4 py-2 text-left border-b'>payslips Name</th>
