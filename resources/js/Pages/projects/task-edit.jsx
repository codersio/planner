...
const EditProject = ({ project, employees, user, user_type, notif }) => {
    const [selectedOptions, setSelectedOptions] = useState(project.assignments.map(assign => assign.employee_id) || []);
    ...
    const handleSubmit = (e) => {
        ...
        const totalEmployeeHours = Object.values(employeeEstimates).reduce((total, hours) => {
        ...
        if (totalEmployeeHours > parseFloat(data.estimate_hours)) {
            ...
            notyf.error(`Total employee hours (${totalEmployeeHours} hours) exceed the task estimate of ${data.estimate_hours} hours.`);
            ...
            <form onSubmit={handleSubmit} className='px-[8rem] grid grid-cols-2 gap-4'>
                ...
                {project.task_name !== 'NON-BILLABLE' && (
                    <div>
                        ...
                        />
                        {errors.estimate_hours && <div>{errors.estimate_hours}</div>}
                    </div>
                )}
                ...
                <div>
                    ...
                    <Multiselect
                        options={employees}
                        selectedValues={employees.filter(employee => selectedOptions.includes(employee.id))}
                        onSelect={(selectedList, selectedItem) => {setData('employee_id', [...data.employee_id, selectedItem.id]);}}
                        onRemove={(selectedList, removedItem) => {setData('employee_id', data.employee_id.filter(id => id !== removedItem.id));}}
                        displayValue="name"
                        className='w-full rounded-lg'
                    />
                ...
                {data.employee_id.map((employeeId) => (
                    <div key={employeeId}>
                        <label htmlFor={`estimate_time_${employeeId}`}>
                            Estimate Time for {employees.find(emp => emp.id === employeeId)?.name}:
                        </label>
                        <input
                            type="text"
                            id={`employee_hours_${employeeId}`}
                            value={employeeEstimates[employeeId] || ''}
                            onChange={(e) => handleEstimateChange(employeeId, e.target.value)}
                            className='w-full rounded-lg'
                        />
