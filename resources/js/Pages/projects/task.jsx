...
const Task = ({ user, tasks, user_type, notif, taskcategory, employee,projects }) => {
    ...
    useEffect(() => {
        ...
        const filtered = allTasks.filter(task =>
            (query === '' || task.task_name.toLowerCase().includes(query.toLowerCase()) || task.users.some(user => user.name.toLowerCase().includes(query.toLowerCase()))) &&
            (projectQuery === '' || tasks.some(project => project.title.toLowerCase().includes(projectQuery.toLowerCase()) && project.tasks.includes(task)))
        );
    ...
    const handleDelete = (e, id) => {
        ...
        if (confirm('Are you sure you want to delete this task?')) {
            axios.get(`/task-delete/${id}`)
                .then(response => {
                    notyf.success('Task and related task assign deleted successfully.');
    ...
    const handleMultipleSelectChange = (selectedList, selectedItem) => {
        setData('employee_id', selectedList.map(item => item.id));
    };
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.get(`/project-tasks/${selectedProjectId}/total-hours`);
            ...
            const projectEstimateTime = parseFloat(selectedProject.estimate_time); // Project's allowed time

            // Validate the total hours against the project estimate time limit
            if (totalHours > projectEstimateTime) {
                notyf.error(`Total hours exceed the allowed limit for the project. Maximum allowed: ${projectEstimateTime} hours.`);
            ...
            post('/task-store', {
                ...
                onSuccess: () => {
                    notyf.success('Task created successfully with notification sent.');
                },
                onError: (errors) => {
            ...
            notyf.error('Failed to validate project hours. Please try again later.');
        }
    };
        ...
        <div className='w-[85.2%] absolute right-0 overflow-hidden'>
            ...
            <div className='px-8 table-section'>
                ...
                <table className="w-full border border-collapse border-gray-200 table-auto">
                    ...
                    <tr>
                        ...
                        <th className="px-4 py-2 border border-gray-300">Assigned Users</th>
                    ...
                    {currentTasks.length > 0 ? (
                        currentTasks.map((task, index) => {
                            const projectName = tasks.find(project => project.tasks.includes(task))?.title || 'Unknown Project';
                            return (
                                <tr key={index}>
                                    <td className="px-4 py-2 border border-gray-300 text-[0.9rem]">{projectName}</td>
                                    <td className="px-4 py-2 border border-gray-300 text-[0.9rem]">{task.task_name}</td>
                                    <td className="px-4 py-2 border border-gray-300">
                                        {task.users.map((user, userIndex) => (
                                            <span className='bg-blue-400 text-white text-xs font-medium me-2 px-2.5 py-0.5 rounded-full' key={userIndex}>
                                                    {user.name} - {user.pivot.employee_hours} hours
                                                {userIndex < task.users.length - 1 && ', '}
                                                </span>
                                        ))}
                                    </td>
                                    <td className="flex px-4 py-2 space-x-2 border border-gray-300">
                                        ...
                                        />
                                        <button
                                            onClick={() => handleCopyClick(projectName)}
                        ...
                        <tr>
                            <td colSpan="4" className='p-3 text-center'>No tasks found</td>
                        </tr>
                    )}
                    </tbody>
                </table>
            ...
            {isModalOpen && (
                <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-50'>
                    <form onSubmit={handleSubmit} className='px-[8rem] items-center justify-center '>
                        <div className='bg-white p-6 rounded-lg shadow-lg'>
                            <h2 className='text-xl font-semibold mb-4'>Copied Project Name</h2>
                            <input
                                type="text"
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    {taskcategory.map(category => (
                                        <option key={category.id} value={category.tname}>{category.tname}</option>
                                    ))}
                                </select>
                            </div>
                            <div>
                            ...
                            <div>
                                <label htmlFor="employee_id">Assign Employees</label>
                                <Multiselect
                                    options={employee}
                                    selectedValues={data.employee_id.map(id => employee.find(e => e.id === id))}
