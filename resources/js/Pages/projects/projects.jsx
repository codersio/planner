        ...
        if (confirm('Are you sure you want to delete this project?')) {
            axios.get(`/projects-delete/${id}`)
                .then(response => {
                    ...
                    notyf.success('Project and related assignments deleted successfully.');
        ...
        <div className='w-[85.2%] absolute right-0 overflow-hidden'>
            ...
            <div className='table-section border-[#0A1B3F] py-3 px-8 rounded-b-md'>
                <div className='flex justify-between'>
                    ...
                    <div className='px-4'>
                        <Link href='projects-create' className='flex space-x-2 underline'>
                        ...
                        {currentProjects.length > 0 ? (
                            currentProjects.map(proj => (
                                <tr key={proj.id}>
                                    ...
                                    <td className='p-3 border'>{proj.start_date}</td>
                                    <td className='p-3 border'>{proj.end_date}</td>
                                    <td className='p-3 border'>{proj.estimate_time}hrs</td>
                                    <td className='border'>
                                        <div className='flex justify-center space-x-3'>
                                            <Link className='text-green-800 text-[1.1rem] bg-[#0C7785] p-1 rounded-md'
                                                  href={`projects-edit/${proj.id}`}>
                                                <CiEdit className='text-white'/>
                                            </Link>
                                            <button className='text-white text-[1.1rem] bg-[#FF3A6E] p-1 rounded-md'
                            ...
                            <tr>
                                <td colSpan="4" className='p-3 text-center'>No projects found</td>
                            </tr>
                        )}
                    </tbody>
                </table>
