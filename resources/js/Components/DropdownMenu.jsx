...
export default function DropdownMenu({ icon, name, items }) {
  ...
  return (
    <li className='p-2 h-full'>
      ...
      <ul ref={ulRef} className="overflow-hidden pl-5 space-y-2 transition-all duration-300" style={{
        maxHeight: `${height}px`,
      ...
      }}>
        {
          items && items.map((item, index) => (
            <li key={index}>
              <Link className='flex items-center text-sm text-gray-600 gap-x-2'>
                <RxDotFilled size={14} />
                <span>{item}</span>
              </Link>
            </li>
